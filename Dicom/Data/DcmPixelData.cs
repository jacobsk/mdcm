...
namespace Dicom.Data
{
    public class DcmPixelData
    {
        ...
        {
            return GetFrameDataUnsigned<byte>(frame);
        }

        public ushort[] GetFrameDataU16(int frame)
        {
            return GetFrameDataUnsigned<ushort>(frame);
        }

        public uint[] GetFrameDataU32(int frame)
        {
            return GetFrameDataUnsigned<uint>(frame);
        }

        private T[] GetFrameDataUnsigned<T>(int frame)
        {
            int typeSize;
            ...
            {
                List<ByteBuffer> fragments = GetFrameFragments(frame);
                int size = fragments.Sum(fragment => fragment.Length) / typeSize;
            ...
            else
            {
                ...
                if (typeSize == 1)
                {
                    if (BytesAllocated > 1 && data.Endian != Endian.LocalMachine)
                        Endian.SwapBytes(BytesAllocated, buffer as byte[]);
                    else if (PixelDataItem.VR == DicomVR.OW && data.Endian == Endian.Big)
                        Endian.SwapBytes(2, buffer as byte[]);
                }
                return buffer;
            }
        }
        ...
        public int[] GetFrameDataS32(int frame)
        {
            ...
            else if (SamplesPerPixel == 3)
            {
                ...
                if (PhotometricInterpretation != "RGB" && PhotometricInterpretation != "YBR_FULL")
                    throw new DicomDataException("PhotometricInterpretation=" + PhotometricInterpretation +
                                                 " is unsupported!");
        ...
        [Obsolete]
        internal byte[] GetFrameDataU8_(int frame)
        {
            if (frame < 0 || frame >= NumberOfFrames)
                throw new IndexOutOfRangeException("Requested frame out of range!");
        ...
        [Obsolete]
        public ushort[] GetFrameDataU16_(int frame)
        {
            if (frame < 0 || frame >= NumberOfFrames)
                throw new IndexOutOfRangeException("Requested frame out of range!");
